...
class ModelFromMysqlCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ducor:modelfrommysql
    ...
    public function __construct()
    {
        ...
        $this->delimiter = config('laravel-generator.model.delimiter', ', ');
    }
    ...
    public function handle()
    {
        ...
        foreach ($tables as $table) {
            ...
            if (!$overwrite and file_exists($fullPath)) {
                $this->error("Model already exists! '$filename.php'");
    ...
    public function checkMysql()
    {
        ...
        if($driver != 'mysql'){
            $this->error("This command only work with mysql");
            exit();
        }
    }
    ...
    public function replaceModuleInformation($stub, $modelInformation)
    {
        ...
        $primaryKey = config('laravel-generator.model.primaryKey', 'id');
    ...
    private function getOption(string $key, $default = null, bool $isBool = false)
    {
        if ($isBool) {
            $return = ($this->option($key))
                ...
                : config("laravel-generator.model.{$key}", $default);
        } else {
            $return = $this->options[$key] = $this->option($key) ?? config("laravel-generator.model.{$key}", $default);
        }
    ...
    {
        $tables = [];
        $whitelist = config('laravel-generator.model.whitelist', []);
        $blacklist = config('laravel-generator.model.blacklist', []);
